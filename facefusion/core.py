...
# #core.py- for optimizations, still introduces artifacts, extracts and merges 750 frames, rest swap and enhancement is as per 'n' frame numbers...
# import itertools
# import shutil
# import signal
# import sys
...
# 	#handles and processes only --frame-start and --frame-end
# 	if temp_frame_paths:
# 		# Get custom frame range parameters
# 		frame_start = state_manager.get_item('frame_start')
# 		frame_end = state_manager.get_item('frame_end')
		
# 		# If frame range is specified, only process frames in that range
# 		if frame_start is not None and frame_end is not None:
# 			# Adjust for trim if needed
# 			if trim_frame_start is not None:
# 				# Calculate relative frame indices based on trim
# 				relative_start = frame_start - trim_frame_start
# 				relative_end = frame_end - trim_frame_start
# 			else:
# 				relative_start = frame_start
# 				relative_end = frame_end
			
# 			# Ensure frame indices are within bounds
# 			relative_start = max(0, relative_start)
# 			relative_end = min(len(temp_frame_paths) - 1, relative_end)
			
# 			# Filter frames to process
# 			frames_to_process = []
# 			for i, frame_path in enumerate(temp_frame_paths):
# 				if relative_start <= i <= relative_end:
# 					frames_to_process.append(frame_path)
			
# 			logger.info(f"Processing only frames {frame_start} to {frame_end} ({len(frames_to_process)} frames)", __name__)
			
# 			# Process only the selected frames
# 			for processor_module in get_processors_modules(state_manager.get_item('processors')):
# 				logger.info(wording.get('processing'), processor_module.__name__)
# 				processor_module.process_video(state_manager.get_item('source_paths'), frames_to_process)
# 				processor_module.post_process()
# 		else:
